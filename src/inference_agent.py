...
class InferenceAgent:
    ...
    def solve(self, question: str) -> str:
        ...
        # 2. Generate Initial Candidate Answer
        initial_answer = ""
        if "MATH" in category:
        ...
        # 3. Reflexion Step (Verify and Refine)
    ...
    def verify_and_refine(self, question: str, initial_answer: str) -> str:
        ...
        if "Error" in initial_answer and "execution failed" in initial_answer:
             # If it's an execution error, we definitely want to try to solve it another way (e.g. CoT)
             # or just ask the model to fix it.
             pass
        ...
        match = re.search(r"\[\[FINAL ANSWER:\s*(.*?)\]\]", critique, re.DOTALL)
        if match:
        ...
        # If model complained but didn't provide format, return original (risky to parse raw text)
        return initial_answer
    ...
    def react_chain(self, question: str, max_turns: int = 5) -> str:
        ...
        for i in range(max_turns):
            response = call_model_chat_completions(
                ...
                system=system_prompt,
                temperature=0.4,
                # Stop generation at Observation so we can insert it
                # Note: API wrapper might not support stop sequences natively if not passed in payload,
                # but the model should stop naturally if instructed well or we cut it off.
                # We'll handle parsing carefully.
            )
            self.call_count += 1
